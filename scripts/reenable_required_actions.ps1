...
$toEnable = @('VERIFY_EMAIL','CONFIGURE_TOTP','UPDATE_PROFILE','UPDATE_PASSWORD','VERIFY_PROFILE')
foreach ($alias in $toEnable) {
    try {
        ...
        if ($match) {
            $body = @{ providerId = $match.providerId; alias = $match.alias; name = $match.name; enabled = $true; defaultAction = $false; priority = $match.priority } | ConvertTo-Json
            ...
            Write-Host "Re-enabled required-action provider: $alias"
        } else {
            Write-Host "Provider $alias not found"
        }
    } catch {
        Write-Host ("Failed to enable {0}: {1}" -f $alias, $_)
    }
}

Write-Host 'Done re-enabling selected required-action providers'
